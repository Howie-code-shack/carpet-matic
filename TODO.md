...
- [x] Phase C — release prep (2026-07-20): App Store listing copy (`AppStore/listing.md`); four 6.9" marketing screenshots (`AppStore/screenshots/`); release archive + distribution-signed .ipa exported successfully (repeatable: `xcodebuild archive` + `-exportArchive` with `-allowProvisioningUpdates`). **Upload blocked on: ASC app record + IAP product + CloudKit schema→Production.**
- [x] Phase C — freemium paywall (2026-07-20): StoreKit 2 `StoreManager` + `PaywallView`; non-consumable `howie.one.CarpetMatic.pro` (£19.99 in `CarpetMatic.storekit`, wired into the shared scheme for simulator testing). Free tier: 1 project, no PDF export; gates on "+" (ProjectListView) and Export (ResultView, "Pro" badge). Store-unavailable state degrades gracefully. **Before release: create the same product ID in App Store Connect.**
- [x] Phase C — competitive quick wins (2026-07-20): pattern repeat (`Project.patternRepeatCM`, engine pads cut lengths to repeat multiples, 5 tests); stair calculator (steps/tread/riser/nosing persisted on `RoomModel`, fills length); underlay m² + gripper m (engine `totalRoomAreaCM2` / `gripperPerimeterCM`, gripper excludes stairs, 2 tests) on result + PDF. 40 engine tests total.
- [x] Phase B — PDF branding + settings (2026-07-20): `BusinessProfileModel` (name/phone/email, SwiftData so it syncs; singleton-by-oldest-`createdAt`); `SettingsView` behind a gear button on `ProjectListView`; `PDFExporter.Branding` header block on exports when set.
- [x] Phase B — cost estimate (2026-07-20): optional "Price per metre" on the project (`pricePerMetrePence: Int`, pence like cm — CloudKit-safe default 0 = hidden); Estimate row on `ResultView` and estimate line on the PDF; `MoneyFormat` helpers in `UI/Helpers.swift`.
